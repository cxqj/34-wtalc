...
def MILL(element_logits, seq_len, batch_size, labels, device):
    ...
    for i in range(batch_size):
        tmp, _ = torch.topk(element_logits[i][:seq_len[i]], k=int(k[i]), dim=0)  #沿给定dim维度返回输入张量input中 k 个最大值 (94,20)
...
def CASL(x, element_logits, seq_len, n_similar, labels, device):
    ...
    for i in range(0, n_similar*2, 2):  # 只计算成对的相似视频的Loss
        ...
        n1 = torch.FloatTensor([np.maximum(seq_len[i]-1, 1)]).to(device)  #749
        n2 = torch.FloatTensor([np.maximum(seq_len[i+1]-1, 1)]).to(device)  #53
        Hf1 = torch.mm(torch.transpose(x[i][:seq_len[i]], 1, 0), atn1)              #(2048,20)
        Hf2 = torch.mm(torch.transpose(x[i+1][:seq_len[i+1]], 1, 0), atn2)          #(2048,20)
        Lf1 = torch.mm(torch.transpose(x[i][:seq_len[i]], 1, 0), (1 - atn1)/n1)     #(2048,20)
        Lf2 = torch.mm(torch.transpose(x[i+1][:seq_len[i+1]], 1, 0), (1 - atn2)/n2) #(2048,20)

        d1 = 1 - torch.sum(Hf1*Hf2, dim=0) / (torch.norm(Hf1, 2, dim=0) * torch.norm(Hf2, 2, dim=0)) #20
        d2 = 1 - torch.sum(Hf1*Lf2, dim=0) / (torch.norm(Hf1, 2, dim=0) * torch.norm(Lf2, 2, dim=0)) #20
        d3 = 1 - torch.sum(Hf2*Lf1, dim=0) / (torch.norm(Hf2, 2, dim=0) * torch.norm(Lf1, 2, dim=0)) #20
...
def train(itr, dataset, args, model, optimizer, logger, device):
    ...
    features, labels = dataset.load_data(n_similar=args.num_similar) # (6,750,2048)/(6,20)
    seq_len = np.sum(np.max(np.abs(features), axis=2) > 0, axis=1) #[264,101,750,463,473,750]  获取有效的特征长度
    features = features[:,:np.max(seq_len),:] #(6,750,2048)

    features = torch.from_numpy(features).float().to(device)
    labels = torch.from_numpy(labels).float().to(device)

    final_features, element_logits = model(Variable(features)) #(6,750,2048)/(6,750,20)
